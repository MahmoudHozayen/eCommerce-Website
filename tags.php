<?php 
	...
	$pagename = "";
		...
		if (isset($_GET['tag-name'])) {
			$pagename = filter_var($_GET['tag-name'], FILTER_SANITIZE_STRING);
		} else {
	...
	<div class="container">
		...
		<?php
			if (isset($_GET['tag-name'])) {
				...
				$items	 	= getAll("*", "items", "Where Tags LIKE '%$tagName%'", "AND Approval_Status	= 1", "Item_ID", "", "");
